...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    //          event handler: 이메일 인풋 키 다운 클릭 이벤트 처리          //
  ...
  const SignUpCard = () => {
    ...
    const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
      ...
      if (code === 'DT') {
        ...
        setTelNumberErrorMessage('중복되는 전화번호 입니다.');
      }
    ...
    const onNextButtonClickHandler = () => {
      ...
      const isCheckedPassword = password.trim().length >= 8 && password.trim().length <= 20;
      if (!isCheckedPassword) {
        setPasswordError(true);
        setPasswordErrorMessage('비밀번호는 8자 이상 20자 이하로 입력해주세요.');
      }
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            <InputBox ref={telNumberRef} label='전화번호*' type='text' placeholder='전화번호를 입력해주세요.' value={telNumber} onChange={onTelNumberChangeHandler} error={isTelNumberError} message={telNumberErrorMessage} onKeyDown={onTelNumberKeyDownHandler} />
          ...
          <div className='auth-card-bottom'>
            {page === 1 && (
            <div className='black-large-full-button' onClick={onNextButtonClickHandler}>{'다음'}</div>
            )}
            {page === 2 && (
            <>
      ...
      <div className='auth-container'>
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            <div className='auth-logo-icon logo-light-icon'></div>
            <div className='auth-jumbotron-text-box'>
              <div className='auth-jumbotron-text'>{'생각을 기록하고'}</div>
              <div className='auth-jumbotron-text'>{'경험을 나누다'}</div>
            </div>
            <div className='auth-jumbotron-subtitle'>{'틈새록과 함께 나만의 기록을 시작해보세요.'}</div>
            <div className='auth-jumbotron-divider'></div>
            <div className='auth-jumbotron-tags'>
              <span className='auth-jumbotron-tag'>{'# 개발'}</span>
              <span className='auth-jumbotron-tag'>{'# 일상'}</span>
              <span className='auth-jumbotron-tag'>{'# 기록'}</span>
            </div>
          </div>
        </div>
